...
const MaterialManager = (function () {
  ...
  function showAnimationDropdown(thumbnailItem, animationType, fileData, onApplyAnimation) {
    ...
    const existingDropdown = document.querySelector('.animation-dropdown');
    if (existingDropdown) {
    ...
    dropdown.style.top = `${thumbnailRect.top + window.scrollY + 40}px`; // Dưới nút animation
    ...
    // Xác định tiêu đề dựa trên loại animation
    let headerText = 'Chọn hiệu ứng';
    if (animationType === 'in') {
    ...
    animationList.className = 'animation-dropdown-list';
    ...
    console.log(`Found ${animations.length} animations for type ${animationType}`);
    ...
    if (animations.length === 0) {
      ...
      animations.forEach(animation => {
        ...
        item.addEventListener('click', () => {
          // Áp dụng animation khi được chọn
          if (typeof onApplyAnimation === 'function') {
            onApplyAnimation(thumbnailItem, animation, animationType);
          }
          ...
          if (currentButton) {
            currentButton.innerHTML = `<i class="${animation.icon || 'fas fa-magic'}"></i> ${animation.name}`;
          }
          ...
          if (animationType === 'group' && animation.animation_id !== 'none') {
            ...
            if (inButton) {
              ...
              if (typeof onApplyAnimation === 'function') {
                onApplyAnimation(thumbnailItem, { animation_id: 'none', name: 'None', icon: 'fas fa-ban' }, 'in');
              }
            }
            ...
            if (outButton) {
              ...
              if (typeof onApplyAnimation === 'function') {
                onApplyAnimation(thumbnailItem, { animation_id: 'none', name: 'None', icon: 'fas fa-ban' }, 'out');
              }
            }
          }
          ...
          if ((animationType === 'in' || animationType === 'out') && animation.animation_id !== 'none') {
            ...
            if (groupButton) {
              ...
              if (typeof onApplyAnimation === 'function') {
                onApplyAnimation(thumbnailItem, { animation_id: 'none', name: 'None', icon: 'fas fa-ban' }, 'group');
              }
            }
          }
